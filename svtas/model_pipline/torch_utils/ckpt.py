...
from svtas.utils.fileio import FileClient, get_file_backend
from svtas.utils.fileio import load as load_file
from svtas.utils.logger import print_log
from svtas.utils.path import mkdir_or_exist
from svtas.utils.package_utils import digit_version
from svtas.utils.misc import apply_to, deprecated_function
...
def _get_svtas_home():
    svtas_home = os.path.expanduser(
        os.getenv(
            ENV_MMENGINE_HOME,
            os.path.join(
                os.getenv(ENV_XDG_CACHE_HOME, DEFAULT_CACHE_DIR), 'svtas')))

    mkdir_or_exist(svtas_home)
    return svtas_home
...
def load_state_dict(module, state_dict, strict=False, logger=None):
    ...
    if len(err_msg) > 0 and rank == 0:
        ...
        if strict:
            ...
            print_log(err_msg, logger=logger, level=LoggerLevel.WARNING)
...
def get_torchvision_models():
    ...
    if digit_version(torchvision.__version__) < digit_version('0.13.0a0'):
        ...
        json_path = osp.join(svtas.__path__[0], 'hub/torchvision_0.12.json')
        model_urls = svtas.utils.fileio.load(json_path)
...
def get_external_models():
    mmengine_home = _get_svtas_home()
    default_json_path = osp.join(svtas.__path__[0], 'hub/openmmlab.json')
...
def get_mmcls_models():
    mmcls_json_path = osp.join(svtas.__path__[0], 'hub/mmcls.json')
...
def get_deprecated_model_names():
    deprecate_json_path = osp.join(svtas.__path__[0], 'hub/deprecated.json')
...
def load_from_openmmlab(filename, map_location=None):
    ...
    if model_url.startswith(('http://', 'https://')):
        ...
        filename = osp.join(_get_svtas_home(), model_url)
        if not osp.isfile(filename):
            raise FileNotFoundError(f'{filename} can not be found.')
        checkpoint = torch.load(filename, map_location=map_location)
    return checkpoint
