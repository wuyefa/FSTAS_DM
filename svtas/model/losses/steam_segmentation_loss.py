...
class StreamSegmentationLoss(BaseLoss):
    ...
    def forward(self, model_output, input_data):
        ...
        # if len(backbone_score.shape) == 3:
            
        backbone_loss = self.backbone_clip_loss({"output":backbone_score.unsqueeze(0)}, backbone_loss_info)['loss']
        # backbone_loss = self.backbone_clip_loss({"output":backbone_score}, backbone_loss_info)['loss']
...
class DiffusionStreamSegmentationLoss(BaseLoss):
    ...
    def forward(self, model_output, input_data):
        ...
        if len(backbone_score.shape) == 3:
            backbone_loss = self.backbone_clip_loss({"output":backbone_score.unsqueeze(0)}, backbone_loss_info)['loss'] # 单阶段
        else:
            backbone_loss = self.backbone_clip_loss({"output":backbone_score}, backbone_loss_info)['loss'] # 多阶段
...
@AbstractBuildFactory.register('loss')
class DiffusionStreamSegmentationLossWithoutVAEBackbone(BaseLoss):
    def __init__(self,
                 vae_backbone_loss_cfg,
                 backbone_loss_cfg,
                 head_loss_cfg,
