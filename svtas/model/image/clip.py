...
from ..utils import TemporalShift_VIT
import numpy as np
import torch
...
class VisionTransformer(nn.Module):
    def __init__(self,
        ...
        if joint:
            logger.info('=====using joint space-time====')
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            if tsm:
                logger  = get_logger("SVTAS")
                logger.info('=========using TSM==========')
                self.make_temporal_shift_vit(self.visual, clip_seg_num)
    ...
    def make_temporal_shift_vit(self, n_segment, n_div=8, place='block', temporal_pool=False):
        ...
        if place == 'block':
            ...
            self.transformer.resblocks = make_block_temporal(self.transformer.resblocks, n_segment_list[0])
...
#             net.layer4 = make_block_temporal(net.layer4, n_segment_list[3])
    ...
    def init_weights(self, child_model=False, revise_keys=[(r'backbone.', r'')]):
        if child_model is False:
            if isinstance(self.pretrained, str):
                logger  = get_logger("SVTAS")
                self._load_model(logger=logger, full_pretrain=True)
            else:
    ...
    def _load_model(self, logger, full_pretrain=True):
        device = "cuda" if torch.cuda.is_available() else "cpu"
        try:
            # loading JIT archive
            state_dict = torch.jit.load(self.pretrained, map_location=device).state_dict()
        except RuntimeError:
            # loading saved state dict
            logger.warn(f"File {self.pretrained} is not a JIT archive. Loading as a state dict instead")
            state_dict = torch.load(self.pretrained, map_location="cpu")
        self.build_model(state_dict, logger=logger, full_pretrain=full_pretrain)
        if str(device) == "cpu":
            self.float()
        ...
        if str(device) == "cpu":
            ...
            self.apply(patch_float)
            patch_float(self.encode_image)
            patch_float(self.encode_text)
    ...
    #                 logger.info(n_k)
        ...
        if full_pretrain:
            ...
            if self.joint:  #or emb_dropout>0 or dropout>0
